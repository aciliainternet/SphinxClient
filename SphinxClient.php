...
class SphinxClient
{
	...
	protected $_ranker;		///< ranking mode (default is self::SPH_RANK_PROXIMITY_BM25)
	protected $_rankexpr;		///< ranking mode expression (for self::SPH_RANK_EXPR)
			...
			while ( $count-->0 && $p<$max )
			{
				...
				foreach ( $attrs as $attr=>$type )
				{
					...
					} else if ( $type==self::SPH_ATTR_MULTI64 )
					{
						...
						while ( $nvalues>0 && $p<$max )
						{
							$attrvals[$attr][] = $this->sphUnpackU64 ( substr ( $response, $p, 8 ) ); $p += 8;
							$nvalues -= 2;
						}
